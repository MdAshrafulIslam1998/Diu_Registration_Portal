...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($input_status)){
        $status_err = "Please enter registration status.";     
    } else{
        $status = $input_status;
    }
    ...
    if(empty($name_err) && empty($status_err) && empty($section_err) && empty($course_err) && empty($roll_err)){
        ...
        $sql = "INSERT INTO student_record (name, status, section, course, roll) VALUES (?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($conn, $sql)){
           
            mysqli_stmt_bind_param($stmt, "sssss", $param_name, $param_status, $param_section, $param_course, $param_roll);           
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
						...
						<div class="form-group <?php echo (!empty($roll_err)) ? 'has-error' : ''; ?>">
                            <label>DIU ID</label>
                            <textarea name="roll" class="form-control"><?php echo $roll; ?></textarea>
                            <span class="help-block"><?php echo $roll_err;?></span>
                        </div>
						
                        <div class="form-group <?php echo (!empty($status_err)) ? 'has-error' : ''; ?>">
                            <label>Registration Status<br></br><br></br></label>																					
                        ...
                        <input type="submit" class="btn btn-primary btn-primary custom-btn" value="Register">
